...
年に1度のセットアップです。**テンプレートを使えばコードのコピーは不要**で、4ステップで完了します。

### テンプレートから始める（推奨）

#### ① テンプレートをコピーする

引き継ぎ担当者から共有されたテンプレートスプレッドシートを開き、「ファイル」→「コピーを作成」で自分の Google ドライブに保存してください。

> テンプレートURLをまだ受け取っていない場合は、前任者またはリポジトリ管理者に確認してください。リポジトリ管理者向けのテンプレート作成手順は後述の「[テンプレートの管理](#テンプレートの管理)」を参照してください。

#### ② 初回セットアップを実行する

コピーしたスプレッドシートを開くと、上部メニューに「**引き継ぎBot**」が自動で表示されます。

「引き継ぎBot」→「① 初回セットアップを実行する」をクリックしてください。

初回は権限承認のダイアログが表示されるので、すべて許可してください（スプレッドシート読み書き / メール送信 / 外部URLアクセス / トリガー作成 / メニュー・ダイアログ表示 の権限が必要です）。

完了すると確認ダイアログが表示され、次のステップへ案内されます。これにより以下が自動で設定されます：

- スプレッドシートに `notifications` シート（通知データ用）と `settings` シート（送信先設定用）が作成される
- 毎朝8時に自動送信を実行するトリガーが登録される

#### ③ ウェブアプリとしてデプロイする

「引き継ぎBot」→「② ウェブアプリのデプロイ手順を確認する」を開くと、手順がダイアログで表示されます。

![ウェブアプリとしてデプロイする画面](./docs/screenshot-step4-deploy.png)

GASエディタを開き、右上「デプロイ」→「新しいデプロイ」から以下の設定でデプロイしてください。
...
> ⚠️ **URLの取り扱い注意**：URL を知っている Google アカウント保有者なら誰でも管理画面に入れます。サークル内で共有する人を絞り、SNS等への公開はしないでください。スプレッドシートそのものは共有不要で、共同編集者は URL だけあれば操作できます（処理はデプロイ者の権限で動きます）。
...
3. `initialize()` やデプロイは**実行しない**（コードが埋め込まれた状態のままにする）
4. スプレッドシートの共有設定を「リンクを知っている全員が閲覧可」にする
5. 取得したURLを README の「テンプレートから始める」セクションに追記する
...
> ⚠️ **URLの取り扱い注意**：URL を知っている Google アカウント保有者なら誰でも管理画面に入れます。サークル内で共有する人を絞り、SNS等への公開はしないでください。
