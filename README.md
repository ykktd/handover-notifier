...
- **送信先を一元管理**：Slack Webhook URL とメールアドレスは設定画面で保存。Webhook URL は保存後に画面へ表示されません
...
4. **設定画面で送信先を登録する**
   ウェブアプリの URL を開き、右上の歯車アイコンから「送信先の設定」へ。Slack Webhook URL と Gmail 送信先メールアドレスを保存して完了です。Webhook URL は保存後に表示されず、登録済みの場合は空欄のまま保存しても変更されません。

> ⚠️ **ウェブアプリURLの取り扱い注意**：URL を知っていれば誰でも管理画面に入れます。サークル内で共有する人を絞り、SNS等への公開はしないでください。
...
- 入力項目：**メッセージ**（必須）／**タイトル**（必須）／**送信日**（必須）／**送信先**（Slack または Gmail）
- メッセージ欄では **`**太字**`** と **`[表示テキスト](https://...)`** の Markdown が使えます。入力欄上部の「太字」「リンク」ボタンから挿入も可能です
...
| 症状                                   | 確認ポイント                                                                                                      |
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| 通知が届かない                         | GASエディタの「実行数」でエラーが出ていないか確認。設定画面で Webhook が「登録済み」か、送信先メールが入力済みかも見る |
...
| URL に `/u/1/` などが勝手に挟まって開けない | 複数の Google アカウントにログインしているとブラウザが自動で挿入する挙動です。URL の末尾に `?authuser=0` を付けるか、共有時にあらかじめ付けて配布してください。シークレットウィンドウで開くのも有効です |
