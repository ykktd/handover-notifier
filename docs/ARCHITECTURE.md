...
| `initialize()` | 初回セットアップ（シート作成 + 日次トリガー登録）を 1 回だけ実行 |
...
| `apiGetSettings()` | 設定（Webhook URL・メールアドレス）を取得 |
...
```
notificationsシート列: id | title | message | date | dest | link | sent
...
| `renderSettings()` | 設定画面（Webhook URL・メール一覧） |
